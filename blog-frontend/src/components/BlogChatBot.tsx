...
function ChatMessageItem({
  ...
  // 思考过程稍快、正文略慢一点，视觉层次更清晰
  const shownReasoning = useTypewriter(msg.reasoning || '', {
    charsPerSecond: 260,
    done: !msg.streaming,
  })
  ...
  // 正文是否已开始显示（决定思考区标题与光标位置）
  const contentStarted = shownContent.length > 0
  const showThinkingCursor = msg.streaming && !contentStarted

  return (
    <div className="flex flex-col items-start">
      {/* 思考过程（推理模型）：灰色小字打字机 */}
      {(shownReasoning || showThinkingCursor) && (
        <div className="max-w-[85%] mb-1 px-2.5 py-1.5 rounded-xl bg-gray-50 border border-gray-100 text-xs leading-relaxed text-gray-400 whitespace-pre-wrap">
          <div className="flex items-center gap-1 mb-0.5 text-gray-400">
            <Icon icon="solar:lightbulb-bold" className="w-3 h-3" />
            <span>{showThinkingCursor ? '思考中…' : '思考过程'}</span>
          </div>
          {shownReasoning}
          {showThinkingCursor && shownReasoning && (
            <span className="inline-block w-1.5 h-3 ml-0.5 bg-gray-300 animate-pulse rounded-sm align-middle" />
          )}
        </div>
      )}

      <div className="max-w-[85%] px-3 py-2 rounded-2xl text-sm leading-relaxed bg-gray-100 text-gray-800 rounded-bl-md">
        {msg.streaming && !contentStarted && !shownReasoning ? (
